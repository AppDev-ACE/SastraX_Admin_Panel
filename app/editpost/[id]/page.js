...
export default function EditPost(){
    ...
    const selectedSchools = school.filter(s => schools.split(",").includes(s.value));
    const selectedDepts = deptOptions.filter(d => dept.split(",").includes(d.value));
        ...
        <div className="relative bg-black min-h-screen mx-auto py-5">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="flex flex-col p-3 mb-4 ml-2 w-68 font-sans text-white text-lg md:w-180 lg:mx-4 lg:w-210 rounded-xl border border-gray-500">
                        <Select value={selectedSchools} onChange={(selected) => setSchools(selected.map(s => s.value).join(","))} instanceId="schools-select" name="schools" options={school} isMulti placeholder="Schools" classNames={{control:()=> "bg-gray-900 border border-gray-500 text-white rounded-lg",valueContainer:()=> "bg-gray-900 text-white gap-1",input:()=> "text-white",placeholder:()=> "text-gray-400",menu:()=> "bg-gray-900",menuList:()=> "bg-gray-900",option:({isFocused,isSelected})=> isSelected?"!bg-gray-600 text-white":isFocused?"!bg-gray-700 text-white":"!bg-gray-900 text-white",multiValue:()=> "!bg-gray-700 !rounded-md px-2",multiValueLabel:()=> "!text-white",multiValueRemove:()=> "!text-white hover:!bg-red-500 hover:!text-white"}}/>
                </div>
                {
